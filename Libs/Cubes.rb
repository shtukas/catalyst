...
class Cub3sX
    ...
    def self.getDatablobOrNull2(uuid, nhash)
        ...
        puts "reading nhash: #{nhash} from cube file".yellow
        filepath = Cub3sX::uuidToFilepathOrNull(uuid)
        raise "(error: bee6247e-c798-44a9-b72b-62773f75254e) uuid: #{uuid}" if filepath.nil?
        datablob = Cub3sX::getDatablobOrNull1(filepath, nhash)

        if datablob then
            nhash2 = Datablobs::putBlob(datablob)
            if nhash2 != nhash then
                puts "something incredibly wrong happened while transferring #{nhash} from the cube '#{filepath}' to datablob repository"
                raise "(error: 34f9ff7e-5e8b-44ff-b18d-86ea44949e3d)"
            end
        end
