...
class NxListings
    ...
    def self.toString(listing)
        "🌌 #{listing["description"].ljust(NxListings::dimension())} #{"%4.2f" % listing["hours-24"]} hours, #{NxListings::itemsInOrder(listing).size.to_s.rjust(6)} items, is priority: #{listing["priority-19"]}"
    end
