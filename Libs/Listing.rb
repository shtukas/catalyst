...
class Listing
    ...
    def self.toString2(store, item)
        ...
        line = "#{storePrefix}#{Listing::listingPositioningAsString(item)} #{PolyFunctions::toString(item)}#{UxPayload::suffix_string(item)}#{NxBalls::nxballSuffixStatusIfRelevant(item)}#{Operations::donationSuffix(item)}#{Operations::parentingSuffix(item)}#{Operations::engineDonationSuffix(item)}"
