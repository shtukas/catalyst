...
class WaveHits
    ...
    def self.waveHit(timespanInSeconds)
        filename = "#{CommonUtils::timeStringL22()}.json"
        filepath = "#{Config::pathToGalaxy()}/DataHub/Catalyst/data/WavesHits/#{filename}"
        data = {
    ...
    # WaveHits::getTotalOfThePastNHours(n)
    def self.getTotalOfThePastNHours(n)
        LucilleCore::locationsAtFolder("#{Config::pathToGalaxy()}/DataHub/Catalyst/data/WavesHits")
            .select{|filepath| filepath[-5, 5] == ".json"}
            .map{|filepath|
                data = JSON.parse(IO.read(filepath))
                if (Time.new.to_i - data["unixtime"]) > 86400 then
            ...
            .map{|filepath|
                data = JSON.parse(IO.read(filepath))
                if (Time.new.to_i - data["unixtime"]) < 3600*n then
                    data["timespan"]
                else
                    0
                end
    ...
    # WaveHits::getRatioOverThePastNHours(n)
    def self.getRatioOverThePastNHours(n)
        WaveHits::getTotalOfThePastNHours(n).to_f/(3600*n)
    end

    # WaveHits::getHigestRatio()
    def self.getHigestRatio()
        [1,2,3].map{|n| WaveHits::getRatioOverThePastNHours(n) }.max
    end
end
