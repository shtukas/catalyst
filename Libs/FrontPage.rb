class FrontPage
    ...
    def self.toString2(store, item)
        ...
        line = "#{storePrefix} #{PolyFunctions::toString(item)}#{UxPayloads::suffixString(item)}#{Parenting::suffix(item)}#{NxBalls::nxballSuffixStatusIfRelevant(item)}#{DoNotShowUntil::suffix(item)}#{TimeCommitments::suffix(item)}"
