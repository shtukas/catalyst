...
pub fn get_mikuType_partial_items_1(conn: &Connection, mikuType: String) -> Result<Vec<PartialItem>, Error> {
    let mut stmt = conn.prepare("select _uuid_, _mikuType_, _item_ from Items where _mikuType_ = ?")?;
    let elements = stmt.query_map(params![mikuType], |row| {
        // Items (_uuid_ string primary key, _mikuType_ string, _item_ string)
        let uuid: String = row.get(0).unwrap();
        let mikuType = row.get(1).unwrap();
        let item_as_json_string: String = row.get(2).unwrap();
...
pub fn get_mikuType_partial_items_2(mikuType: String) -> Vec<PartialItem> {
    let conn = Connection::open("/Users/pascal/Galaxy/DataHub/Catalyst/data/Items/20240607-155704-609823.sqlite3").unwrap();
    get_mikuType_partial_items_1(&conn, mikuType).unwrap()
}